...
import { MetricPoint, WorkDistribution } from '@/types/metrics';
...
export async function getWorkDistributionMetrics(): Promise<WorkDistribution> {
  const data = await getMetrics();
  return data.workDistribution;
}
